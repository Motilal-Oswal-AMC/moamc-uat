...
export default function decorate(block) {
  ...
  dataMapMoObj.GetFilter().then((resp) => {
    schemeNames = resp.map((fund) => fund.schDetail.schemeName);
  ...
  let selectedFund;
  dataMapMoObj.GetFilter().then((resp) => {
    selectedFund = resp.find((fund) => fund.schcode === schcode);
    let returnCAGR = 0;
    ...
    searchInput.addEventListener('input', (e) => {
      ...
      filtered.forEach((name) => {
        ...
        lione.addEventListener('click', () => {
          ...
          selectedFund = resp.find((f) => f.schDetail.schemeName === name);
          searchResults.innerHTML = '';
          updatePlanOptions(selectedFund);
          updateReturnRate();
          // calContainer.querySelector('.cancel-btn').style.display = 'block';
